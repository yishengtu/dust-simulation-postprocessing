from dust_simulation_athdf_hydro_parallel_time_tracer_enabled import *
...
if __name__ == '__main__':
    ...
    # fn_root_pars = './'
    fn_root_pars = 'H:\\StarFormation\\test_sim4\\small_hole_more_frames\\dust_1e-1\\'
    ...
    num_par =          int(input_params["num_par"])
    ...
    print ('number of cores using = ' + str(config.NUMBA_NUM_THREADS))
    ...
    athdf_obj_list = []
    while 0 < len(athdf_output_fn_list):
    ...
    if restart:
        ...
        grain_init_loc_list = init_grain(npar = num_par, x1min = 20 * AU, x1max = 5000 * AU, nx1 = 128, x2min = 0.01*np.pi, x2max=0.99*np.pi, nx2=64, rho_fun=athdf_obj_list[0].rho_fun)
        grain_list = []
    ...
    while ot < T:
        ...
        grid.set_hydro_with_time(new_time_list=new_time_list) # only process info needed in the next loop

        time_passed = time.time() - start_time
        hour = int(time_passed / 3600)
        minute = int((time_passed - 3600 * hour) / 60)
        second = int(time_passed - 3600 * hour - 60 * minute)
        print ('Set hydro completed. The number of stored frames is ' + str(len(grid.rho_mesh)) + '     number of grains in domain is ' + str(grain_list.shape[0]) +  '   Elapsed time: ' + '{0:4d} hour, {1:2d} min, {2:2d} sec'.format(hour, minute, second))
