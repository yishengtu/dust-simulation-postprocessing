from V2_4_more_interp import kb, mH, h, sigma, a, m_sun, AU, year, l_sun, G, c, mu, gamma
from V2_4_more_interp import Util, OutputAthdf, SphericalPolarCoord
from V2_4_more_interp import arctan, TSC, apply_TSC, init_grain, move
import numpy as np
import time
...
if __name__ == '__main__':
    ...
    fn_root_pars = './'
    # fn_root_pars = 'H:\\StarFormation\\test_sim4\\small_hole_more_frames\\dust_test\\'
    ...
    par_num =          int(input_params["par_num"])
    ...
    if restart:
        ot, ini_time_in_athena_check, frame_number, grain_list, hydro_x, hydro_y, hydro_rho = Util.read_restart_file_npy(fn_root_pars + restart_file)    # read in particle locations
        ...
        grid.set_hydro_with_time(new_time_list=np.array([athdf_obj_list[0].hydro_time, athdf_obj_list[1].hydro_time]), fill_value=fill_value)
        grain_init_loc_list = init_grain(npar = par_num, x1min = par_rmin, x1max = par_rmax, nx1 = 320,
                                         x2min = par_thetamin, x2max=par_thetamax, nx2=128,
                                         athdf_rho_mesh=athdf_obj_list[0].cgs_rho_mesh,
                                         athdf_r_list=athdf_obj_list[0].cgs_r_list,
                                         athdf_theta_list=athdf_obj_list[0].theta_list)
        grain_list = []
        for grain_init_loc in grain_init_loc_list:
            ...
            new_grain = [x, y, z, gas_vx, gas_vy, gas_vz, s, rho, particle_type]
        ...
        Util.write_restart_file_npy(fn_root_pars + 'frame' + Util.choosenumber(frame_number) + '', grain_list=grain_list, ot=ot, ini_time_in_athena=ini_time_in_athena, frame_number=frame_number,
                                    hydro_x=grid.x_mesh, hydro_y=grid.y_mesh, hydro_rho=grid.rho_mesh[0])
    ...
    while ot < T:
        ...
        Util.write_restart_file_npy(fn_root_pars + 'frame' + Util.choosenumber(frame_number) + '', grain_list=grain_list, ot=ot, ini_time_in_athena=ini_time_in_athena, frame_number=frame_number,
                                    hydro_x=grid.x_mesh, hydro_y=grid.y_mesh, hydro_rho=grid.rho_mesh[-1])